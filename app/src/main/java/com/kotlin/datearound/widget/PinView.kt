...
class PinView(context: Context) : View(context) {
    ...
    fun init(ctx: Context, y: Float, pinRadiusDP: Float, pinColor: Int, textColor: Int,
        ...
        mPinPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15f,
                mRes!!.displayMetrics).toInt().toFloat()
        mCircleRadiusPx = circleRadius
        mTextYPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3.5f,
                mRes!!.displayMetrics).toInt().toFloat()
        // If one of the attributes are set, but the others aren't, set the
        // attributes to default
        if (pinRadiusDP == -1f) {
            mPinRadiusPx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_THUMB_RADIUS_DP,
                    mRes!!.displayMetrics).toInt()
        } else {
            mPinRadiusPx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, pinRadiusDP,
                    mRes!!.displayMetrics).toInt()
        }
